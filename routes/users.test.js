...
describe("POST /users", function () {
  ...
  test("bad request if missing data, created by Admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
  ...
  test("bad request if invalid data, created by Admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users", function () {
  ...
  test("fails: test next() handler, by Admin", async function () {
...
describe("GET /users/:username", function () {
  test("works for Admin, incorrect user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(200);
...
describe("PATCH /users/:username", () => {
  ...
  test("works: set new password by correct user", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
